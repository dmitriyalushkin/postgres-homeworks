...
SELECT WHERE ship_country IN ('France', 'Germany', 'Spain')
FROM orders;
...
SELECT order_date, AVG(shipped_date), WHERE ship_country = 'Germany'
FROM orders;
...
SELECT MIN(unit_price) MAX(unit_price) WHERE discontinued != 1
FROM products;
...
SELECT MIN(unit_price) MAX(unit_price) WHERE discontinued != 1 AND units_in_stock >= 20
FROM products;